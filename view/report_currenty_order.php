...
class PDF extends FPDF{//create class
    ...
    function TableHeading()///table heading function
    {
        ...
        $this->Cell(190,0,'CURRENT YEAR ORDER REPORT',0,0,'C');
        $this->Ln();
        $this->Cell(190, 0, "___________________________________", 0, 0, "C");
    }
    function TableBody(){///table body function
            ...
            $currentyearResult1=$OrderReportObj->getCurrentYearAllOrderReport();///get result order report model
            $currentYearResult2=$OrderReportObj->getCurrentYearAllOrderPlace();///get result order report model
            $row1 = $currentYearResult2->fetch_assoc();///make a result as an array value
            $this->Cell(0,10,'',0,1);
            $this->SetFont("Arial","B","10"); 
            $this->Cell(32,10,'TOTAL ORDER PLACE :',0,"",'L');
            $this->Cell(40,10,$row1["allorder"],0,"1",'C');
            ...
            $this->SetFont("Arial","","10");
            $counter++;
        while($row=$currentyearResult1->fetch_assoc()){

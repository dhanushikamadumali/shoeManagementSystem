...
class PDF extends FPDF{//create class
    ...
    function TableHeading()///table heading function
    {
        ...
        $this->Cell(190,0,'LAST TWO MONTH ORDER REPORT',0,0,'C');
        $this->Ln();
        $this->Cell(190, 0, "______________________________________", 0, 1, "C");
        
    }
    function TableBody(){///table body function
        include '../model/Order_Report_Model.php';///include order report 
        $OrderReportObj = new orderReport();///create order report object
        $TwoMonthResult1=$OrderReportObj->getLastTwoMAllOrderReport();///get result order report model
        $TwoMonthResult2=$OrderReportObj->getLastTwoMAllOrderPlace();///get result order report model
        $row1 = $TwoMonthResult2->fetch_assoc();///make a result as an array value
        $this->Cell(0,10,'',0,1);
        ...
        $this->SetFont("Arial","","10");
        $counter++;
        while($row=$TwoMonthResult1->fetch_assoc()){

...
class PDF extends FPDF{//create class
    ...
    function TableHeading()///table heading function
    {
        ...
        $this->Cell(190,0,'LAST SEVEN DAY INCOME REPORT',0,0,'C');
        $this->Ln();
        $this->Cell(190, 0, "_____________________________________", 0, 0, "C");
    }
    function TableBody(){///table body function
        include '../model/Income_Report_Model.php';//include report model
        $incomeReportObj = new incomeReport();//crerate income object
        $WeeklyResult = $incomeReportObj->getWeeklyReport();//get result income model
        $WeeklyEVResult = $incomeReportObj->getWeeklyEarnV();//get result income model
        $EVrow = $WeeklyEVResult->fetch_assoc();///make a result as an array value
            $this->Cell(0,10,'',0,1);
            $this->Cell(0,10,'',0,1);

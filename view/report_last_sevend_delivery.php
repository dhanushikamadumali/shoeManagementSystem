...
class PDF extends FPDF{//create class
    ...
    function TableHeading()///table heading function
    {
        ...
        $this->Cell(190,0,'LAST SEVEN DAY DELIVERY REPORT',0,0,'C');
        $this->Ln();
        $this->Cell(190, 0, "_______________________________________", 0, 0, "C");
    }
    function TableBody(){///table body function
        ...
        $SevenDayResult1=  $deliveryReportObj->getWeeklyReport();///get result order report model
        $SevenDayResult2=  $deliveryReportObj->getWeeklyCustomedateSuccDelivery();///get result order report model
        $row1 = $SevenDayResult2->fetch_assoc();///make a result as an array value
        $this->Cell(0,10,'',0,1);
        $this->SetFont("Arial","","10"); 
        $this->Cell(32,10,'ALL SUCCESS DELIVERED :',0,"",'L');
        $this->Cell(60,10,$row1["successdelivery"],0,"",'C');
        ...
        $this->SetFont("Arial","","10");
        $counter++;
        while($row=$SevenDayResult1->fetch_assoc()){

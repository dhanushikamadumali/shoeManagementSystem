...
class PDF extends FPDF{//create class
    ...
    function TableHeading()///table heading function
    {
        ...
        $this->Cell(190,0,'DAY ORDER REPORT',0,0,'C');
        $this->Ln();
        $this->Cell(190, 0, "_______________________________", 0, 0, "C");
    }
    function TableBody(){///table body function
        ...
        $customeDayResult1= $OrderReportObj->getCustomedateAllOrderReport($fromdate,$todate);//pass value order model
        $customeDayResult2 =  $OrderReportObj->getCustomedateAllOrderPlace($fromdate,$todate);//pass value order model
        $row1 =  $customeDayResult2->fetch_assoc();/////make a result as an array value
        $this->Cell(0,10,'',0,1);
        $this->SetFont("Arial","B","10"); 
        $this->Cell(32,10,'TOTAL ORDER PLACE :',0,"",'L');
        $this->Cell(60,10,$row1["allorder"],0,"1",'C');
        $this->SetFont("Arial","B","10"); 
        ...
        $this->SetFont("Arial","","10");
        $counter++;
        while($row= $customeDayResult1->fetch_assoc()){

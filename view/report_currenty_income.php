...
class PDF extends FPDF{//create class
    ...
    function TableHeading()///table heading function
    {
        ...
        $this->Cell(190,0,'CURRENT YEAR INCOME REPORT',0,0,'C');
        $this->Ln();
    ...
    function TableBody(){///table body function
        ...
        $crrentyearResult=$incomeReportObj->getCurrentYearReport();///get result income model
        $currentYearEVResult=$incomeReportObj->getCurrentYearEarnV();///get result income model
        $EVrow = $currentYearEVResult->fetch_assoc();///make a result as an array value
        $this->Cell(0,10,'',0,1);
        $this->Cell(0,10,'',0,1);
        ...
        $this->SetFont("Arial","","10");
        $counter++;
        while($row=$crrentyearResult->fetch_assoc()){

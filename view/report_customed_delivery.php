...
class PDF extends FPDF{//create class
    ...
    function TableHeading()///table heading function
    {
        ...
        $this->Cell(190,0,'DAY DELIVERY REPORT',0,0,'C');
        $this->Ln();
        $this->Cell(190, 0, "___________________________", 0, 0, "C");
    }
    function TableBody(){///table body function
        ...
        $CustomedResult1=  $deliveryReportObj->getCustomedateReport($fromdate,$todate);///get result order report model
        $CustomeResult2=  $deliveryReportObj->getCustomedateSuccDelivery($fromdate,$todate);///get result order report model
        $row1 = $CustomeResult2->fetch_assoc();///make a result as an array value
        $this->Cell(0,10,'',0,1);
        $this->SetFont("Arial","B","10"); 
        $this->Cell(32,10,'ALL SUCCESS DELIVERED :',0,"",'L');
        $this->SetFont("Arial","","10"); 
        $this->Cell(50,10,$row1["successdelivery"],0,"1",'C');
        $this->SetFont("Arial","B","10"); 
        $this->Cell(10,10,'DATE :',0,"",'L');
        $this->SetFont("Arial","","10"); 
        $this->Cell(30,10,$fromdate,0,"",'C');
        $this->Cell(15,10,'to',0,"",'L');
        $this->Cell(10,10,$todate,0,"1",'C');
        ...
        $this->SetFont("Arial","","10");
        $counter++;
        while($row=$CustomedResult1->fetch_assoc()){

...
class PDF extends FPDF{//create class
    ...
    function TableHeading()///table heading function
    {
        ...
        $this->Cell(190,0,'LAST TWO MONTH INCOME REPORT',0,0,'C');
        $this->Ln();
        $this->Cell(190, 0, "______________________________________", 0, 0, "C");
    }
    function TableBody(){///table body function
        include '../model/Income_Report_Model.php';//include model
        $incomeReportObj = new incomeReport();//create report object
        $twoMonthResult= $incomeReportObj->getLastTwoMReport();//get report detail
        $TwoMonthEVResult= $incomeReportObj->getLastTwoMEarnV();//get earn value result
        $EVrow = $TwoMonthEVResult->fetch_assoc();//make a result as an array value
        $this->Cell(0,10,'',0,1);
        $this->Cell(0,10,'',0,1);
        ...
        $this->SetFont("Arial","","10");
        $counter++;
        while($row=$twoMonthResult->fetch_assoc()){
        ...
        $this->Cell(38,10,$EVrow["earnvalue"].'.00',1,"1",'C');

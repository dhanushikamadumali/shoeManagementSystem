...
class PDF extends FPDF{//create class
    ...
    function TableHeading()///table heading function
    {
        ...
        $this->Cell(190,0,'OUT OF STOCK REPORT',0,0,'C');
        $this->Ln();
        $this->Cell(190, 0, "_________________________", 0, 1, "C");
    }
    function TableBody(){///table body function
        ...
        $Result1=$StockReportObj ->getoutofStockReport();///get result order report model
        $Result2=$StockReportObj ->getoutofStock();///get result order report model
        $row1 = $Result2->fetch_assoc();///make a result as an array value
        
        $this->Cell(0,10,'',0,1);
        $this->Cell(0,10,'',0,1);
        $this->SetFont("Arial","B","10"); 
        $this->Cell(32,10,'P_ID',1,"",'C');
        $this->Cell(32,10,'P_NAME',1,"",'C');
        $this->Cell(32,10,'SIZE',1,"",'C');
        $this->Cell(32,10,'A_QUNTY',1,"",'C');
        ...
        while($row=$Result1->fetch_assoc()){
            $this->Cell(32,10,$row["product_product_id"],1,"",'C');
            $this->Cell(32,10,$row["product_name"],1,"",'C');
            $this->Cell(32,10,$row["size"],1,"",'C');
            $this->Cell(32,10,$row["a_quantity"],1,"",'C');

...
class PDF extends FPDF{//create class
    ...
    function TableHeading()///table heading function
    {
        ...
        $this->Cell(190,0,'LAST TWO MONT DELIVERY REPORT',0,0,'C');
        $this->Ln();
    ...
    function TableBody(){///table body function
        ...
        $twoMonthResult1=$deliveryReportObj->getLastTwoMReport();///get result report model
        $TwoMonthResult2=$deliveryReportObj->getLastTwoMSuccDelivery();///get result report model
        $row1 = $TwoMonthResult2->fetch_assoc();///make a result as an array value
        $this->Cell(0,10,'',0,1);
        $this->SetFont("Arial","","10"); 
        $this->Cell(32,10,'ALL SUCCESS DELIVERED :',0,"",'L');
        $this->Cell(45,10,$row1["successdelivery"],0,"",'C');
